...
Hosted inspection found that raw point exports contain `Pts`, `TbSet`, `1st` and `2nd`, but omit many spreadsheet convenience fields described in the dictionary. Missing first-serve-in, ace, double-fault and rally-count labels remain null; original serve strings remain available for later verified parsing. When `TB?` is absent, a conservative score-syntax derivation distinguishes ordinary games from tiebreaks. The opening 0–0 point of a tied late-set game remains unknown until a currently observed score resolves the mode; propagation is forward only within the game, never from later outcomes. `TbSet` is not substituted for a current-point tiebreak indicator. These incomplete mode flags limit descriptive coverage.
