...
## Hosted pilot outcome

The reduced [court-context-36183174897-1 pilot](https://github.com/kennynakao/Tabular-Model-for-sports/releases/tag/court-context-36183174897-1) is published as a partial prerelease. It used three label-screened tennis facilities from the pinned prior Wikidata release, approximately 600 m boxes, and January 1 snapshots in 2020 and 2024. Two of four attempted Overpass queries succeeded, yielding 18 element/place/snapshot observations. These are not a count of distinct physical courts.

Every exported observation lacks all four requested attributes: surface, indoor, covering and lighting. The pilot therefore does **not** establish usable coverage for those context fields. It preserves element identity, mapped center, version and snapshot/edit metadata, with uncertain facility associations and no automatic match joins. Two element centers outside the query boxes were rejected. A timeout followed by HTTP 429 stopped the remaining two planned queries; no retries or alternate endpoints were attempted. Do not immediately repeat this source after its rate-limit response.

The run transferred 767,029 bytes across six actual HTTP requests, including the source archive and its redirect. Eighteen synthetic tests and hosted CI passed. OpenStreetMap rows remain separately attributed under ODbL; selected Wikidata coordinates remain CC0. All processing and CSV storage occurred in GitHub-hosted infrastructure.
