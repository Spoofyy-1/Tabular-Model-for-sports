...
The [tennis weather alignment pilot](https://github.com/kennynakao/Tabular-Model-for-sports/releases/tag/tennis-weather-36187582212-1) remains **audit only**: 35 CC0 structured claims, zero accepted match joins and zero joined weather buckets. A [frozen-input diagnostic](docs/TENNIS_ALIGNMENT_DIAGNOSTIC_RELEASE.md) found no exact candidate-day rows in either source-export or normalized match metadata; combining source aliases added no matches. The other candidate lacks a required court claim. Existing daily weather remains separate; no playing-hour exposure is inferred. [Source contracts and exclusions](docs/TENNIS_WEATHER_ALIGNMENT_RESEARCH.md).
